...
#include "Result/ResultSerializer.h"
...
int main() {
    std::cout << "=== TEST WYDAJNOSCI: Move Semantics vs Kopiowanie ===" << std::endl;
    std::cout << std::endl;

    // Test 1: Operator+ BEZ move semantics (zwraca przez wartosc - kopia)
    std::cout << "--- Test 1: Operator+ BEZ move semantics ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1, t2, t3;
        t1.enter("+ 1 2");
        t2.enter("* 3 4");

        // operator+ zwraca przez wartosc, wiec tworzy kopie
        t3 = t1 + t2;  // Kopia: t1+t2 tworzy tymczasowy obiekt, potem kopiuje do t3
        std::cout<<t3.print().getValue()<<"\n";
    }

    std::cout << "Bez move: ";
    EquationTree::printCopyStats();

    // Test 2: Operator+ Z move semantics
    std::cout << "\n--- Test 2: Operator+ Z move semantics ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1, t2, t3;
        t1.enter("+ 1 2");
        ...
        // Uzywamy std::move na wyniku operator+
        t3 = std::move(t1 + t2);  // Przeniesienie: tymczasowy obiekt jest przenoszony, nie kopiowany
        std::cout<<t3.print().getValue()<<"\n";
    }
    std::cout << "Z move: ";
    EquationTree::printCopyStats();

    // Test 3: Przypisanie BEZ move semantics
    std::cout << "\n--- Test 3: Przypisanie BEZ move semantics ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1, t2;
        t1.enter("+ 1 2");
        t2 = t1;  // Kopiowanie
        std::cout<<t2.print().getValue()<<"\n";
    }
    std::cout << "Bez move (przypisanie): ";
    EquationTree::printCopyStats();

    // Test 4: Przypisanie Z move semantics
    std::cout << "\n--- Test 4: Przypisanie Z move semantics ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1, t2;
        t1.enter("+ 1 2");
        t2 = std::move(t1);  // Przeniesienie
        std::cout<<t2.print().getValue()<<"\n";
    }
    std::cout << "Z move (przypisanie): ";
    EquationTree::printCopyStats();

    // Test 5: Zwracanie z funkcji BEZ move
    std::cout << "\n--- Test 5: Konstruktor kopiujacy ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1;
        t1.enter("+ 1 2");
        EquationTree t2(t1);  // Kopiowanie przez konstruktor
        std::cout<<t2.print().getValue()<<"\n";
    }
    std::cout << "Konstruktor kopiujacy: ";
    EquationTree::printCopyStats();

    // Test 6: Konstruktor przenoszacy
    std::cout << "\n--- Test 6: Konstruktor przenoszacy ---" << std::endl;
    EquationTree::resetCopyCount();
    {
        EquationTree t1;
        t1.enter("+ 1 2");
        EquationTree t2(std::move(t1));  // Przeniesienie przez konstruktor
        std::cout<<t2.print().getValue()<<"\n";
    }
    std::cout << "Konstruktor przenoszacy: ";
    EquationTree::printCopyStats();

    // Resetuj licznik przed dalszym działaniem programu
    EquationTree::resetCopyCount();
